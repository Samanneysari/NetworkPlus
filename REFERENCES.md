# Official References

This course paraphrases and teaches the material; it does not copy standards or vendor documentation. Access date: 2026-08-25. Current CompTIA registration information always takes precedence.

## Exam

- [CompTIA Network+ N10-009 Exam Objectives v4.0 (PDF)](https://comptiacdn.azureedge.net/webcontent/docs/default-source/exam-objectives/comptia-network-n10-009-exam-objectives-(4-0)-(1).pdf)
- [CompTIA Network+ certification page](https://www.comptia.org/certifications/network)

## Protocols and architecture

- [RFC Editor index](https://www.rfc-editor.org/)
- [IPv4 — RFC 791](https://www.rfc-editor.org/rfc/rfc791)
- [ARP — RFC 826](https://www.rfc-editor.org/rfc/rfc826)
- [ICMP — RFC 792](https://www.rfc-editor.org/rfc/rfc792)
- [Internet host communication requirements — RFC 1122](https://www.rfc-editor.org/rfc/rfc1122)
- [IPv6 — RFC 8200](https://www.rfc-editor.org/rfc/rfc8200)
- [TCP — RFC 9293](https://www.rfc-editor.org/rfc/rfc9293)
- [UDP — RFC 768](https://www.rfc-editor.org/rfc/rfc768)
- [TLS 1.3 — RFC 8446](https://www.rfc-editor.org/rfc/rfc8446)
- [Private IPv4 addressing — RFC 1918](https://www.rfc-editor.org/rfc/rfc1918)
- [IPv4 /31 point-to-point links — RFC 3021](https://www.rfc-editor.org/rfc/rfc3021)
- [IPv6 SLAAC — RFC 4862](https://www.rfc-editor.org/rfc/rfc4862)
- [DNS concepts — RFC 1034](https://www.rfc-editor.org/rfc/rfc1034)
- [DNS implementation — RFC 1035](https://www.rfc-editor.org/rfc/rfc1035)
- [DNSSEC — RFC 4033](https://www.rfc-editor.org/rfc/rfc4033)
- [DHCP — RFC 2131](https://www.rfc-editor.org/rfc/rfc2131)
- [NTPv4 — RFC 5905](https://www.rfc-editor.org/rfc/rfc5905)
- [Network Time Security — RFC 8915](https://www.rfc-editor.org/rfc/rfc8915)
- [VXLAN — RFC 7348](https://www.rfc-editor.org/rfc/rfc7348)
- [Zero Trust Architecture — NIST SP 800-207](https://csrc.nist.gov/pubs/sp/800/207/final)

## Tools and implementation

- [Wireshark User's Guide](https://www.wireshark.org/docs/wsug_html_chunked/)
- [BIND 9 Administrator Reference Manual](https://bind9.readthedocs.io/)
- [OpenSSH manuals](https://www.openssh.com/manual.html)
- [Cisco Packet Tracer](https://www.netacad.com/cisco-packet-tracer)
- [Microsoft Windows commands](https://learn.microsoft.com/windows-server/administration/windows-commands/windows-commands)
- [Microsoft netstat command](https://learn.microsoft.com/windows-server/administration/windows-commands/netstat)
- [Microsoft workgroup protocol glossary](https://learn.microsoft.com/openspecs/windows_protocols/ms-brws/f95bb7d5-6c67-4957-bebc-41687f07e21a)
- [Microsoft domain-join guidance](https://learn.microsoft.com/windows-server/identity/ad-ds/manage/join-computer-to-domain)
- [Cisco MAC address-table configuration guide](https://www.cisco.com/c/en/us/td/docs/switches/lan/c9000/lyr2-fwd/cdp-lldp-mac-udld/cdp-lldp-mac-udld-configuration-guide/configure-mac.html)
- [Linux iproute2 manual](https://man7.org/linux/man-pages/man8/ip.8.html)
- [Linux netstat manual and `ss` replacement note](https://man7.org/linux/man-pages/man8/netstat.8.html)

RFCs are standards, not beginner textbooks. Read the relevant lesson first, then use the RFC or official vendor manual to confirm exact protocol or platform behavior.
